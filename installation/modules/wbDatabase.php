...
class wbDatabase {
  ...
  **********************************************************/
  public function prepareQuery( $query, $data ){
    if (is_string($query) && (is_array($data) || is_object($data))) {
      foreach ($data AS $k => $v) {
        if (is_numeric($v) || is_real($v)) {
          $query = str_replace(':'.$k, $v, $query);
        }
        else if (is_bool($v)) {
          $query = str_replace(':'.$k, (int)$v, $query);
        }
        else if (is_string($v)) {
          $query = str_replace(':'.$k, "'" . $this->getEscaped($v) . "'", $query);
        }
      }
    }
    return $query;
  ...
  public function &runQuery( $query, $data = array() ){
    ...
    if( strlen($this->_query) ){
      ...
      if( $this->_driver == 'pdo' ){
        ...
        $this->_result = mysqli_query($this->_dbh, $this->prepareQuery( $this->_query, $data ));
        if( !$this->_result )
          $this->_throwError();
      }
      else {
        ...
        $this->_result = mysql_query($this->prepareQuery( $this->_query, $data ), $this->_dbh);
        if( !$this->_result )
          $this->_throwError();
      }
    }
  ...
  private function _throwError( $msg=null, $num=400 ){
    if( !is_null($msg) ){
      ...
      if($this->_errorNum < 500){
        ...
        echo '<div class="error_msg">'. $this->_query .'</div>';
        return;
      }
    }
